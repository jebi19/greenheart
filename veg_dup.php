...
// Query to select products with category 'Vegetables'
$sql = "SELECT id, productName, quantity, price, image_path FROM products WHERE category = 'Vegetables'";
...
<body>
            ...
            <tbody id="productList">
                ...
                if ($result->num_rows > 0) {
                    while($row = $result->fetch_assoc()) {
                        echo "<tr>";
                        echo "<td><img src='" . $row["image_path"] . "' alt='" . $row["productName"] . "' style='width:100px; height:auto;'></td>";  // Display image
                    ...
                    echo "<tr><td colspan='5'>No products found.</td></tr>";  // Adjust colspan to match the number of columns
                }
                $conn->close();
                ?>
            </tbody>
    ...
    <script>
        ...
        function proceedToCart() {
            ...
            rows.forEach(row => {
                const productName = row.cells[1].textContent;  // Update to match the new column index
                const quantity = row.querySelector('input[type="number"]').value;
                const price = row.cells[3].textContent;  // Update to match the new column index
